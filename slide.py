...
def main():
	...
	BASICFONT = pygame.font.SysFont('timesnewroman', BASICFONTSIZE)
	#store the option buttons and their rectangles in OPTIONS
	RESET_SURF, RESET_RECT = makeText('Reset', TEXTCOLOR, TILECOLOR, WINDOWWIDTH - 120, WINDOWHEIGHT - 90)
	NEW_SURF, NEW_RECT = makeText('New Game', TEXTCOLOR, TILECOLOR, WINDOWWIDTH - 120, WINDOWHEIGHT - 60)
	SOLVE_SURF, SOLVE_RECT = makeText('Solve', TEXTCOLOR, TILECOLOR, WINDOWWIDTH - 120, WINDOWHEIGHT - 30)
	mainBoard, solutionSeq = generateNewPuzzle(80)
	#solved board is same as the board in a start state
	SOLVEDBOARD = getStartingBoard()
	#list of moves made from the solved configuration
	allMoves = []
	#main game loop
	while True:
		#direction, if any tiles should slide
		slideTo = None
		#contains the message to show in the upper left screen
		msg = ''
...
def getStartingBoard():
	...
	'''for example, if BOARDWIDHT and BOARDHEIGHT are both 3,
...
def getBlankPosition(board):
	#return the x and y of the board coordinates of the blank space
	for x in range(BOARDWIDTH):
		for y in range(BOARDHEIGHT):
			if board[x][y] == None:
				return(x, y)
...
def drawTile(tilex, tiley, number, adjx = 0, adjy = 0):
	'''draw a title at board coordinates tilex and tiley, optionally
	a few pixels over (determined by adjx and adjy)'''
	left, top = getLeftTopOfTile(tilex, tiley)
...
def slideAnimation(board, direction, message, animationSpeed):
	...
	if direction == UP:
		...
		for i in range(0, TILESIZE, animationSpeed):
			#animate the tiles sliding over
			checkForQuit()
...
def generateNewPuzzle(numSlides):
	'''from a starting configuration, make numSlides number
...
def resetAnimation(board, allMoves):
	#make all of moves in allMoves in reverse.
